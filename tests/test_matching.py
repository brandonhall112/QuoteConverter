from pathlib import Path

import pandas as pd

from followup_quotes.config import RunConfig
from followup_quotes.matching import run_matching


def test_option_b_only_followups_with_grouped_order_totals():
    quotes = pd.DataFrame(
        {
            "Quote #": ["Q1", "Q2", "Q3"],
            "Customer Name": ["Acme, Inc.", "Acme Inc", "Beta LLC"],
            "Amount": [2000, 2500, 3000],
            "Quote Date": ["2024-01-01"] * 3,
            "Created By": ["Reid Kincaid"] * 3,
            "Rev": ["A", "B", "X"],
        }
    )
    orders = pd.DataFrame(
        {
            "Order Number": [10, 10, 20],
            "Customer": ["ACME INC", "ACME INC", "BETA LLC"],
            "Net Amount": [1000, 1000, 3000],
            "Revision": ["A", "A", "Y"],
            "Open": ["YES", "YES", "TRUE"],
        }
    )
    qmap = {
        "quote_number": "Quote #",
        "customer": "Customer Name",
        "quote_amount": "Amount",
        "date_quoted": "Quote Date",
        "entry_person_name": "Created By",
        "rev": "Rev",
    }
    omap = {
        "order_id": "Order Number",
        "customer": "Customer",
        "net": "Net Amount",
        "rev": "Revision",
        "open": "Open",
    }
    cfg = RunConfig(
        quotes_path=Path("q.xlsx"),
        orders_path=Path("o.xlsx"),
        out_path=Path("x.xlsx"),
        reps=["Reid Kincaid"],
    )

    out = run_matching(quotes, orders, qmap, omap, cfg)

    assert set(out.followups["Quote"]) == {"Q2"}
    assert "Won by Follow Up?" in out.followups.columns
    assert out.followups["Won by Follow Up?"].eq(False).all()


def test_relative_tolerance_matches_customer_order_totals_without_id_link():
    quotes = pd.DataFrame(
        {
            "Quote #": ["Q-WON", "Q-UNCONVERTED"],
            "Customer": ["Acme", "Acme"],
            "Amount": [5000, 5600],
            "Date Quoted": ["2024-01-01", "2024-01-01"],
            "Entry Person Name": ["Reid Kincaid", "Reid Kincaid"],
        }
    )
    orders = pd.DataFrame(
        {
            "Order Number": ["100", "100", "300"],
            "Customer": ["ACME", "ACME", "ACME"],
            "Net Amount": [2400, 2500, 4300],
        }
    )
    qmap = {
        "quote_number": "Quote #",
        "customer": "Customer",
        "quote_amount": "Amount",
        "date_quoted": "Date Quoted",
        "entry_person_name": "Entry Person Name",
    }
    omap = {"order_id": "Order Number", "customer": "Customer", "net": "Net Amount"}

    strict_cfg = RunConfig(
        quotes_path=Path("q.xlsx"),
        orders_path=Path("o.xlsx"),
        out_path=Path("x.xlsx"),
        reps=["Reid Kincaid"],
        tolerance=25,
        relative_tolerance=0.0,
    )
    relaxed_cfg = RunConfig(
        quotes_path=Path("q.xlsx"),
        orders_path=Path("o.xlsx"),
        out_path=Path("x.xlsx"),
        reps=["Reid Kincaid"],
        tolerance=25,
        relative_tolerance=0.03,
    )

    strict = run_matching(quotes, orders, qmap, omap, strict_cfg)
    relaxed = run_matching(quotes, orders, qmap, omap, relaxed_cfg)

    assert set(strict.followups["Quote"]) == {"Q-WON", "Q-UNCONVERTED"}
    assert set(relaxed.followups["Quote"]) == {"Q-UNCONVERTED"}
